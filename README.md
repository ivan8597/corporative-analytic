...
```
├── data/                           # CSV-данные (генерируются скриптом)
├── docker-compose.yml              # PostgreSQL + Metabase
├── docs/
│   ├── отчёт_для_защиты.md         # полный отчёт для защиты проекта
│   └── выводы_и_рекомендации.md    # выводы по метрикам
├── scripts/
│   ├── generate_data.py            # Генератор синтетических данных
│   ├── setup_metabase.py           # Настройка Metabase и дашборда
│   ├── update_dashboard_insights.py # Текст «Выводы → Рекомендации»
│   ├── install_git_hooks.sh        # Защита от Co-authored-by: Cursor
│   └── git-hooks/                  # Шаблоны git-хуков
└── sql/
    ├── init/                       # Схема и загрузка при старте Postgres
    └── metrics/                    # 5 SQL-запросов для дашборда
```
